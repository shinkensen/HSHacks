...
const CLAP_DISTANCE_THRESHOLD = 0.06
const CLAP_RELEASE_THRESHOLD = 0.1
const CLAP_TOGGLE_COOLDOWN_MS = 1200
...
export default function Workout() {
  ...
  const clapLatchedRef = useRef(false)
  const lastClapToggleAtRef = useRef(0)
  ...
  async function pushRoomPresence() {
    ...
    await fetch(`/api/steps/${encodeURIComponent(roomId.trim())}/landmarks`, {
      method: 'POST',
  ...
  useEffect(() => {
    for (const feed of remoteMediaFeeds) {
      for (const track of feed.stream.getAudioTracks()) {
        track.enabled = isSpeakerEnabled
      }
    }
  }, [remoteMediaFeeds, isSpeakerEnabled])
  ...
  useEffect(() => {
    ...
    void fetch(`/api/steps/${encodeURIComponent(roomId.trim())}/landmarks`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        deviceId: ensureDeviceId(),
    ...
    async function pullRoomFeeds() {
      try {
        const response = await fetch(
          `/api/steps/${encodeURIComponent(roomId.trim())}/landmarks`,
          { cache: 'no-store' },
        )
  ...
  async function sendSignal(
    ...
    await fetch(`/api/steps/${encodeURIComponent(normalizedRoomId)}/signals`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  function startSignalPolling(activeRoomId: string) {
    ...
    const poll = async () => {
      try {
        const response = await fetch(
          `/api/steps/${encodeURIComponent(activeRoomId)}/signals?deviceId=${encodeURIComponent(ensureDeviceId())}&since=${signalCursorRef.current}`,
          { cache: 'no-store' },
        )
        if (!response.ok) {
          return
  ...
  async function createRoom() {
    ...
    try {
      const response = await fetch(`/api/steps/${encodeURIComponent(normalizedRoomId)}/landmarks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          deviceId: ensureDeviceId(),
  ...
  async function joinRoom() {
    ...
    try {
      ...
      await fetch(`/api/steps/${encodeURIComponent(normalizedRoomId)}/landmarks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          deviceId: ensureDeviceId(),
  ...
  function stopCamera() {
    ...
    clapLatchedRef.current = false
    lastClapToggleAtRef.current = 0
  }
  ...
  function renderLoop() {
    ...
    if (hResult.landmarks && hResult.landmarks.length > 0) {
      ...
      detectClapAndToggleMic(hResult.landmarks, t)
        ...
        videoRef.current.onloadedmetadata = async () => {
          ...
          setStatusText(
            stream.getAudioTracks().length > 0
              ? 'Camera active. Voice chat ready.'
              : 'Camera active. Microphone unavailable; voice chat off.',
          )
  ...
  async function toggleMic(trigger: 'button' | 'clap' = 'button') {
    const stream = streamRef.current
    if (!stream) {
      setStatusText('Start camera first to use voice chat.')
      return
    }
    ...
    if (audioTracks.length === 0) {
      try {
        ...
        setStatusText(
          trigger === 'clap'
            ? 'Microphone enabled via clap. Voice chat is live.'
            : 'Microphone enabled. Voice chat is live.',
        )
      } catch {
        setStatusText('Microphone access denied. Voice chat unavailable.')
      }
      return
    }
    ...
    if (trigger === 'clap') {
      setStatusText(nextEnabled ? 'Clap detected: microphone unmuted.' : 'Clap detected: microphone muted.')
    } else {
      setStatusText(nextEnabled ? 'Microphone unmuted.' : 'Microphone muted.')
    }
  }

  function detectClapAndToggleMic(hands: { x: number; y: number; z: number }[][], nowMs: number) {
    if (!streamRef.current || hands.length < 2) {
      clapLatchedRef.current = false
      return
    }
    ...
    const handDistance = Math.hypot(firstWrist.x - secondWrist.x, firstWrist.y - secondWrist.y)
    const canToggle = nowMs - lastClapToggleAtRef.current >= CLAP_TOGGLE_COOLDOWN_MS

    if (handDistance <= CLAP_DISTANCE_THRESHOLD && !clapLatchedRef.current && canToggle) {
      clapLatchedRef.current = true
      lastClapToggleAtRef.current = nowMs
      void toggleMic('clap')
      return
    }

    if (handDistance >= CLAP_RELEASE_THRESHOLD) {
      clapLatchedRef.current = false
    }
  }
      ...
      <section className="border-b border-border/60">
        <div className="mx-auto w-full max-w-5xl px-6 py-12 sm:px-8 sm:py-14">
          ...
          <p className="mt-3 max-w-2xl text-sm text-muted-foreground sm:text-base">
            Real-time form feedback, rep counting, and optional room sharing with peers.
          </p>
          <p className="mt-2 text-xs text-muted-foreground">Status: {statusText}</p>
        </div>
      </section>

      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-8 sm:px-8 sm:py-10">
        <div className="relative overflow-hidden rounded-2xl border border-border bg-card/70">
          <video
            ref={videoRef}
            className="aspect-video w-full bg-muted/40 object-cover"
            playsInline
            muted
          />
          <canvas
          ...
          {showShareCameraAlert && (
            <div className="absolute left-1/2 top-3 z-30 -translate-x-1/2 rounded-md border border-border bg-card/90 px-3 py-2 text-xs text-card-foreground shadow-sm backdrop-blur-sm">
              Camera feed is shared with participants in this room.
            </div>
          )}

          <div className="pointer-events-none absolute right-3 top-3 z-20 w-56 rounded-md border border-border bg-card/90 p-2 shadow-sm backdrop-blur-sm">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Leaderboard</p>
            <div className="mt-1 space-y-1">
              {leaderboard.slice(0, 5).map((entry, index) => (
                ...
                >
                  ...
                  <span className="font-semibold text-primary">{entry.reps}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-3 md:grid-cols-5">
          <div className="rounded-xl border border-border bg-card/70 p-4">
            <p className="text-xs text-muted-foreground">Score</p>
            <p className="mt-1 text-2xl font-medium">{qualityScore}</p>
          </div>
          <div className="rounded-xl border border-border bg-card/70 p-4">
            <p className="text-xs text-muted-foreground">Reps</p>
            <p className="mt-1 text-2xl font-medium">{repCount}</p>
          </div>
          <div className="rounded-xl border border-border bg-card/70 p-4">
            <p className="text-xs text-muted-foreground">Timer</p>
            <p className="mt-1 text-2xl font-medium">{formatDuration(elapsedSeconds)}</p>
          </div>
          <div className="rounded-xl border border-border bg-card/70 p-4">
            <p className="text-xs text-muted-foreground">Hands</p>
            <p className="mt-1 text-2xl font-medium">{handsDetected}</p>
          </div>
          <div className="rounded-xl border border-border bg-card/70 p-4">
            <p className="text-xs text-muted-foreground">Room peers</p>
            <p className="mt-1 text-2xl font-medium">{remoteFeeds.length}</p>
          </div>
        </div>

        <div className="rounded-2xl border border-border bg-card/70 p-4 sm:p-5">
          <h2 className="text-lg font-medium tracking-tight">Cloud Room Relay</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Use the same room ID on all devices. Relay runs through this server.
          </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button
              onClick={joinRoom}
              disabled={roomJoinState === 'creating' || roomJoinState === 'joining'}
              className="inline-flex h-9 items-center rounded-md border border-border bg-background px-4 text-sm font-medium transition-colors hover:bg-accent disabled:opacity-50"
            >
              {roomJoinState === 'joining' ? 'Joining...' : 'Join Room'}
            </button>
            <button
              onClick={() => {
                void toggleMic('button')
              }}
              disabled={!isCameraOn}
              className="inline-flex h-9 items-center rounded-md border border-border bg-background px-4 text-sm font-medium transition-colors hover:bg-accent disabled:opacity-50"
            >
            ...
            >
              {isSpeakerEnabled ? 'Mute Peers' : 'Unmute Peers'}
            </button>
          </div>

          <p className="mt-3 text-xs text-muted-foreground">Room status: {roomProgressText}</p>
          <p className="mt-1 text-xs text-muted-foreground">Gesture: clap both hands in frame to mute or unmute your mic.</p>
          <p className="mt-1 text-xs text-muted-foreground">Device ID: {ensureDeviceId()}</p>
        </div>

        {remoteMediaFeeds.length > 0 && (
          <div className="rounded-2xl border border-border bg-card/70 p-4 sm:p-5">
            <h2 className="text-lg font-medium tracking-tight">Peer Camera Feeds</h2>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              {remoteMediaFeeds.map((feed) => (
                <div key={feed.deviceId} className="overflow-hidden rounded-xl border border-border bg-background">
                  <video
                  ...
                  />
                  <p className="px-3 py-2 text-xs text-muted-foreground">Peer: {feed.deviceId}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          <button
            onClick={startCamera}
            disabled={isCameraOn}
            className="inline-flex h-10 items-center rounded-md bg-primary px-5 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-50"
          >
            Start Camera
          </button>
          <button
            onClick={stopCamera}
            disabled={!isCameraOn}
            className="inline-flex h-10 items-center rounded-md border border-border bg-background px-5 text-sm font-medium transition-colors hover:bg-accent disabled:opacity-50"
          >
            Stop Camera
          </button>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card/70 p-5 sm:p-6">
          <h3 className="text-xl font-medium tracking-tight">Coaching Feedback</h3>
