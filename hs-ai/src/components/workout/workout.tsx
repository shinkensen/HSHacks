...
function resolveDeviceId() {
  if (typeof window === "undefined") return "dev-pending";
...
export default function Workout() {
  ...
  function getRoomPayload() {
    const stableDeviceId = deviceIdRef.current;
    return {
  ...
  const visibleRemoteFeeds = shareEnabled && roomId.trim() ? remoteFeeds : [];
  ...
  function startSignalPolling(activeRoomId: string) {
    ...
    const poll = async () => {
      try {
        const response = await fetch(
          `/api/steps/${encodeURIComponent(activeRoomId)}/signals?deviceId=${encodeURIComponent(deviceIdRef.current)}&since=${signalCursorRef.current}`,
          { cache: "no-store" },
        );
        if (!response.ok) {
          return;
  ...
  async function createRoom() {
    ...
    try {
      const response = await fetch(
        `/api/steps/${encodeURIComponent(normalizedRoomId)}/landmarks`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            deviceId: deviceIdRef.current,
            username:
              username.trim() || `User-${deviceIdRef.current.slice(-4)}`,
            reps: repCount,
            updatedAt: Date.now(),
            poseLandmarks: [],
            handLandmarks: [],
  ...
  function renderLoop() {
    ...
    if (!video || !canvas || !poseLandmarker || !handLandmarker) {
      console.log("RenderLoop skipped: missing refs:", {
        video: !!video,
        canvas: !!canvas,
        pose: !!poseLandmarker,
        hand: !!handLandmarker,
  ...
  async function toggleMic(trigger: "button" | "clap" = "button") {
    ...
    if (trigger === "clap") {
      setStatusText(
        nextEnabled
          ? "Clap detected: microphone unmuted."
          : "Clap detected: microphone muted.",
      );
    } else {
  ...
  function detectClapAndToggleMic(
    ...
    const handDistance = Math.hypot(
      firstWrist.x - secondWrist.x,
      firstWrist.y - secondWrist.y,
    );
    const canToggle =
      nowMs - lastClapToggleAtRef.current >= CLAP_TOGGLE_COOLDOWN_MS;

    if (
      handDistance <= CLAP_DISTANCE_THRESHOLD &&
      !clapLatchedRef.current &&
      canToggle
    ) {
      ...
      <section className="border-b border-border/60">
        <div className="mx-auto w-full max-w-5xl px-6 py-12 sm:px-8 sm:py-14">
          ...
          <p className="mt-3 max-w-2xl text-sm text-muted-foreground sm:text-base">
            Real-time form feedback, rep counting, and optional room sharing
            with peers.
          </p>
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-8 sm:px-8 sm:py-10">
        ...
        <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-3 md:grid-cols-5">
          ...
          <div className="rounded-xl border border-border bg-card/70 p-4">
            ...
            <p className="mt-1 text-2xl font-medium">
              {visibleRemoteFeeds.length}
            </p>
          </div>
        </div>

        <div className="rounded-2xl border border-border bg-card/70 p-4 sm:p-5">
          <h2 className="text-lg font-medium tracking-tight">Cloud Room</h2>
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Room status: {roomProgressText}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Gesture: clap both hands in frame to mute or unmute your mic.
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Device ID: {deviceId}
          </p>
