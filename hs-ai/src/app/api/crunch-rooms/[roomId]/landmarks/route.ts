...
const globalStore = globalThis as typeof globalThis & {
  __hsAiCrunchRoomStore?: Map<string, InMemoryRoomStore>;
};

if (!globalStore.__hsAiCrunchRoomStore) {
  globalStore.__hsAiCrunchRoomStore = new Map<string, InMemoryRoomStore>();
}

const rooms = globalStore.__hsAiCrunchRoomStore;
...
export async function GET(
  ...
  try {
    const devices = await queryConvex<DeviceFeed[]>("crunchRooms:getDevices", {
      roomId: normalizedRoomId,
    });
    const leaderboard = await queryConvex<LeaderboardEntry[]>(
      "crunchRooms:getRoomLeaderboard",
      {
        roomId: normalizedRoomId,
      },
    );
...
export async function POST(
  ...
  try {
    await mutateConvex<{ ok: boolean }>("crunchRooms:upsertDevice", {
      roomId: normalizedRoomId,
