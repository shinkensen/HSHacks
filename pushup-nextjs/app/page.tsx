...
export default function Home() {
  ...
  function renderLoop() {
    ...
    if (Math.random() < 0.05) {
      // Log occasionally for debugging so we don't spam
      console.log(
        "Detection running.",
        "Poses:",
        pResult.landmarks?.length || 0,
        "Hands:",
        hResult.landmarks?.length || 0,
      );
    }
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-neutral-800 p-4 rounded-lg space-y-3">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {remoteMediaFeeds.map((feed) => (
                <div
                  key={feed.deviceId}
                  className="rounded border border-neutral-700 overflow-hidden bg-black"
                >
                  <video
        ...
        <div className="flex gap-4 justify-center">
          <button
            onClick={startCamera}
            disabled={isCameraOn}
            className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 rounded font-semibold disabled:opacity-50 text-white"
          >
            Start Camera
          </button>
          <button
            onClick={stopCamera}
            disabled={!isCameraOn}
            className="px-6 py-2 bg-red-600 hover:bg-red-500 rounded font-semibold disabled:opacity-50 text-white"
          >
            Stop Camera
          </button>
        </div>
